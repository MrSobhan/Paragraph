...
const FollowingPage = () => {
  ...
  const loadUserAndFollowing = async () => {
    ...
    if (result.success) {
      ...
      // In a real app, you'd have an API endpoint to get following users
          ...
          <div className="flex items-center space-x-4 space-x-reverse">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white">
                دنبال شده‌های {user?.name}
              </h1>
              <p className="text-gray-600 dark:text-gray-400 mt-2">
                {following.length} کاربر دنبال شده
              </p>
            </div>
          </div>
        </div>
              ...
              {following.map((followedUser) => (
                ...
                >
                  <div className="flex items-center space-x-4 space-x-reverse mb-4">
                    <button onClick={() => navigate(`/user/${followedUser._id}`)}>
                      <img
                        src={followedUser.avatar || "https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp&f=y"}
                    ...
                    <div className="flex-1">
                      <button
                        onClick={() => navigate(`/user/${followedUser._id}`)}
                        className="text-right hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                      >
                        <h3 className="font-bold text-gray-900 dark:text-white">
                          {followedUser.name}
                        </h3>
                        <p className="text-gray-600 dark:text-gray-400">@{followedUser.username}</p>
                      </button>
                      {followedUser.bio && (
                  ...
                  {currentUser && currentUser._id !== followedUser._id && (
                    ...
                    >
                      {followingStates[followedUser._id] ? <UserCheck className="w-4 h-4" /> : <UserPlus className="w-4 h-4" />}
                      <span>{followingStates[followedUser._id] ? 'دنبال می‌کنید' : 'دنبال کردن'}</span>
                    </button>
                  )}
                </div>
              ))}
